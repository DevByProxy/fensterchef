...
typedef struct window_state {
    ...
    bool is_visible;
    /* if the window can be focused through `_NET_ACTIVE_WINDOW` */
...
struct fensterchef_window {
     ...
     * `reference.id` set to `None`.  All other struct members are invalid at
     * that point.
     */
    ...
    /* use `get_window_border_size()` to get an accurate value */
    unsigned border_size;
    uint32_t border_color, border_color_active, border_color_focus;
    /* radius of the outer and inner border */
     ...
     * updated by the window module but the synchronization module.
     */
...
extern FcWindow *Window_last_activated;
 ...
 *
 * @should_run_relations controls whether any relations should be re-run on the
 *                       window.
 *
 * @return if the atom exists on the window.
 */
bool cache_window_property(FcWindow *window, Atom atom,
        bool should_run_relations);

/* return value for `match_window_pattern()` */
enum match_window_pattern_state {
    /* the window matches the pattern */
    WINDOW_PATTERN_MATCH,
    /* the window does NOT match the pattern */
    WINDOW_PATTERN_MISMATCH
};

/* Match a window against a pattern.
 *
 * @return one of the above constants to indicate the match state. 
 */
enum match_window_pattern_state match_window_pattern(const WindowPattern *pattern,
        FcWindow *window);
 ...
 * the window.
 *
 * @is_wm_state_expected requires the window to have the WM_STATE property,
 *                       otherwise this function returns NULL.
 */
FcWindow *create_window(Window id, bool is_wm_state_expected);

/* Destroy given window and remove it from the window linked list.
...
/* Get a window with given @number or NULL if no window has that number. */
 ...
 *
 * This is not always `window->border_size` since fullscreen windows have no
 * border for example.
 *
...
/* Change the mode to given value and reconfigure the window if it is visible.
 *
 ...
 *
 * The window will later be shown on screen once the X server is synchronized
