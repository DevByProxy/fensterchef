...
struct frame {
    ...
    /* The window inside the frame, may be NULL.  It might be destroyed.
     * To check this use `window->client.id` which should be `None` when the
     * window was destroyed.  This case is important for stashed frames.
     */
    FcWindow *window;
    ...
    /* remember the center axis positions of frames this frame was focused from
     * so that a frame can give back the focus to the same frame it received
     * focus from
     */
    int focused_from_above_x, focused_from_below_x,
        focused_from_left_y, focused_from_right_y;
 ...
 *        number, split direction and ratio @with has.
 ...
 *        taken out.
 *
 * Consider using `link_into_stash()` on the return value.
 *
 ...
 *         this beside checking if it is NULL.
 */
