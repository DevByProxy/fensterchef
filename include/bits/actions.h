...
#define DEFINE_ALL_ACTIONS \
    ...
    /* set the background color of the fensterchef windows */ \
    ...
    X(BORDER_COLOR_ACTIVE_CURRENT, "border color active current I") \
    /* set the active border color of the current window the configured default value */ \
    X(BORDER_COLOR_ACTIVE_CURRENT_DEFAULT, "border color active current default") \
    /* set the border size of the current window */ \
    X(BORDER_SIZE_CURRENT, "border size current I") \
    /* set the border size of the current window */ \
    X(BORDER_SIZE_CURRENT_DEFAULT, "border size current default") \
    ...
    X(BORDER_COLOR_FOCUS_CURRENT, "border color focus current I") \
    /* set the focus border color of the current window */ \
    X(BORDER_COLOR_FOCUS_CURRENT_DEFAULT, "border color focus current default") \
    /* set the border color of all windows */ \
    X(BORDER_COLOR, "border color I") \
    /* set the border color of the current window */ \
    X(BORDER_COLOR_CURRENT, "border color current I") \
    /* set the border color of the current window */ \
    X(BORDER_COLOR_CURRENT_DEFAULT, "border color current default") \
    /* set the border size of all windows */ \
    X(BORDER_SIZE, "border size I") \
    /* set the radius of the window borders */ \
    X(BORDER_RADIUS, "border radius I") \
    /* set the radius of the window borders */ \
    X(BORDER_RADIUS_CURRENT, "border radius current I") \
    /* set the radius of the window borders */ \
    X(BORDER_RADIUS_CURRENT_DEFAULT, "border radius current default") \
    /* set the inner radius of the window borders */ \
    X(BORDER_RADIUS_INNER, "border radius inner I") \
    /* set the inner radius of the window borders */ \
    X(BORDER_RADIUS_INNER_CURRENT, "border radius inner current I") \
    /* set the inner radius of the window borders */ \
    X(BORDER_RADIUS_INNER_CURRENT_DEFAULT, "border radius inner current default") \
    ...
    X(EXCHANGE_WINDOW_DOWN, "exchange window down") \
    /* put the current window on the left monitor */ \
    X(EXCHANGE_WINDOW_LEFT, "exchange window left") \
    /* put the current window on the right monitor */ \
    X(EXCHANGE_WINDOW_RIGHT, "exchange window right") \
    /* put the current window on the monitor above */ \
    X(EXCHANGE_WINDOW_UP, "exchange window up") \
    ...
    X(MOVE_WINDOW_DOWN, "move window down") \
    /* move the current window to the left */ \
    X(MOVE_WINDOW_LEFT, "move window left") \
    /* move the current window to the right */ \
    X(MOVE_WINDOW_RIGHT, "move window right") \
    /* move the current window up */ \
    X(MOVE_WINDOW_UP, "move window up") \
    ...
    X(RESTORE_LAYOUT_S, "restore layout S") \
    /* run a shell program */ \
    X(RUN, "run S") \
    ...
    X(SHOW_APPLICATIONS, "show applications") \
    ...
    /* update the fensterchef executable */ \
    X(UPDATE, "update") \
\
