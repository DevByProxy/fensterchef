...
/* Get a long window property constisting of a specific number of longs.
 *
 * @return a pointer to the list of longs.  Use `XFree()` on this.
 */
long *get_long_property(Window window, Atom property,
        unsigned long expected_item_count);

/* Get an atom window property (Wrapper around `get_long_property()`. */
Atom *get_atom_property(Window window, Atom property,
        unsigned long expected_item_count);

/* Get a text window property.
 *
 * @return a string thay may not be nul-terminated.  Use `XFree()` on this.
 */
