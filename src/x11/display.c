...
/* first index of a shape event (ShapeNotify) */
int shape_event_base = -1;
...
void open_connection(void)
{
    ...
    /* the value is useless because the shape extension has no errors */
    if (XShapeQueryExtension(display, &shape_event_base, &useless_value)) {
        LOG_DEBUG("shape extension is supported\n");
    }
...
void query_existing_windows(void)
{
    ...
    for (unsigned i = 0; i < number_of_children; i++) {
        /* we expect WM_STATE which means this window was managed before by a
         * window manager
         */
        (void) create_window(children[i], true);
    }
