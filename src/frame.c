...
/* Make the frame ready to be freed and dereference it. */
void destroy_frame(Frame *frame)
{
    ...
    if (frame == Frame_focus) {
        ...
        /* fall back to the root of the main monitor */
...
void indicate_frame(Frame *frame)
{
    if (frame->number == 0) {
        ...
        (void) snprintf(number, sizeof(number), "%" PRIu32, frame->number);
...
Frame *get_frame_at_position(int x, int y)
{
    ...
    for (Monitor *monitor = Monitor_first; monitor != NULL;
        ...
        if (is_point_in_frame(frame, x, y)) {
            /* recursively move into child frames until we are at a leaf */
...
/* Get the frame on the left or above of @frame depending on @direction. */
...
/* Get the frame right or below @frame depending on @direction. */
...
void exchange_frames(Frame *from, Frame *to)
{
    ...
    temporary_frame = *from;
    replace_frame(from, to);
    replace_frame(to, &temporary_frame);
}
        ...
        unsigned width, unsigned height)
{
    ...
    case FRAME_SPLIT_HORIZONTALLY:
        left_size = frame->ratio.denominator == 0 ? width / 2 :
                (unsigned long) width * frame->ratio.numerator /
    ...
    case FRAME_SPLIT_VERTICALLY:
        left_size = frame->ratio.denominator == 0 ? height / 2 :
                (unsigned long) height * frame->ratio.numerator /
                    frame->ratio.denominator;
        ...
        unsigned width, unsigned height)
{
    ...
    case FRAME_SPLIT_HORIZONTALLY:
        ...
        left_size = left->width == 0 || right->width == 0 ? width / 2 :
                (unsigned long) width * left->width /
    ...
    case FRAME_SPLIT_VERTICALLY:
        ...
        left_size = left->height == 0 || right->height == 0 ? height / 2 :
              (unsigned long) height * left->height /
...
static Frame *find_frame_void_within(Frame *frame)
{
    ...
    child = find_frame_void_within(frame->left);
    if (child != NULL) {
        return child;
    }

    return find_frame_void_within(frame->right);
}
...
Frame *stash_frame_later(Frame *frame)
{
    ...
    hide_and_reference_inner_windows(frame);

    Frame *const stash = create_frame();
    stash->x = frame->x;
    stash->y = frame->y;
    stash->width = frame->width;
    stash->height = frame->height;
...
/* Derefence @frame, all child frames and any inner windows. */
static void free_frame_recursively(Frame *frame)
{
    if (frame->left != NULL) {
        free_frame_recursively(frame->left);
        free_frame_recursively(frame->right);
        /* we set these to NULL as well as values below because we just
         * dereference the frame which might mean it is still used somewhere
         */
        frame->left = NULL;
        frame->right = NULL;
    } else if (frame->window != NULL) {
...
{
    for (Frame *frame = Frame_last_stashed, *previous;
            frame != NULL;
            frame = previous) {
        previous = frame->previous_stashed;
        free_frame_recursively(frame);
    }
