...
typedef enum program_option {
    ...
    PROGRAM_OPTION_RESTORE_LAYOUT, /* --restore-layout */
} program_option_t;
...
static const struct program_parse_option {
    ...
    [PROGRAM_OPTION_RESTORE_LAYOUT] = { '\0', "restore-layout", 0 },
};
...
/* set by --restore-layout */
static bool should_restore_layout;
...
static void handle_program_option(program_option_t option, char *value)
{
    ...
    case PROGRAM_OPTION_COMMAND: {
        ...
        for (int i = program_argument_index;
            ...
            command[length] = ' ';
            length++;
            COPY(&command[length], program_arguments[i], n);
            length += n;
            command[length] = '\0';
        }
    ...
    /* restore the previously dumped layout */
    case PROGRAM_OPTION_RESTORE_LAYOUT:
        should_restore_layout = true;
        break;
...
static int open_next_log_file(void)
{
    ...
    (void) strftime(time_buffer, sizeof(time_buffer), "%F_%T.log", tm);
    path = allocate_state_path(time_buffer);
    if (path == NULL) {
        fprintf(stderr, "could not create the state directory: %s\n",
                strerror(errno));
        return ERROR;
    }
...
int main(int argc, char **argv)
{
    ...
    user_id = getuid();
    if (user_id == 0) {
        /* fensterchef is run as root user? */
        fprintf(stderr, "fensterchef is not allowed to be run as root user\n");
        return EXIT_FAILURE;
    }

    Fensterchef_executable = argv[0];

    home = getenv("HOME");
    if (home == NULL || home[0] == '\0') {
        passwd = getpwuid(user_id);
        Fensterchef_home = xstrdup(passwd->pw_dir);
        fprintf(stderr, "HOME not set, falling back to %s\n",
                passwd->pw_dir);
    } else {
        Fensterchef_home = xstrdup(home);
    }
    ...
    /* restoring was requested through --restore-layout */
    if (should_restore_layout) {
        ...
        result = restore_frames_and_windows(path);
        if (result == DUMP_IO_ERROR) {
            ...
            LOG_ERROR("file %s is not a valid dump file: %s\n",
                    path, get_string_of_dump_parse_error(result));
        }
    }
