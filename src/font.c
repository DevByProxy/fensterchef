...
void free_font_list(void)
{
    ...
    /* future proof and make `free_font_list()` safer by not leaving invalid
     * pointers accessible (and pretending they are accessible)
     */
    font_list.fonts = NULL;
    font_list.count = 0;
}
...
Text *create_text(const FcChar32 *glyphs, int glyph_count)
{
    ...
    for (int i = 0; i < text->item_count; i++) {
        ...
        if (i + 1 == text->item_count) {
            text->width += MAX(item->extents.width, item->extents.xOff);
