...
int next_cycle(void)
{
    ...
    if (XPending(display) || wait_for_file_descriptor() > 0) {
        ...
        if (old_focus_frame != Frame_focus ||
                (Frame_focus->window == Window_focus &&
                    old_focus_window != Window_focus)) {
            /* if a covering window is focused, show no indication */
            if (Window_focus != NULL &&
                    (focused_monitor = get_focused_monitor(),
                        is_window_covering_monitor(focused_monitor,
                            Window_focus))) {
                /* do nothing */
             ...
             */
            } else if (Frame_focus->window == NULL ||
    ...
    if (Fensterchef_is_update_requested) {
        update_fensterchef();
    }
...
static void handle_map_request(XMapRequestEvent *event)
{
    ...
    window = get_fensterchef_window(event->window);
    if (window == NULL) {
        window = create_window(event->window, false);
        if (window == NULL) {
            LOG("not managing %w\n", event->window);
        }
    }
...
/* We get a map notify every time a window is mapped. */
static void handle_map_notify(XMapEvent *event)
{
    /* check for a special fensterchef command window */
    if (event->override_redirect) {
        check_for_external_command(event->window);
    }
}
...
static void handle_property_notify(XPropertyEvent *event)
{
    ...
    window = get_fensterchef_window(event->window);
    if (window != NULL) {
        (void) cache_window_property(window, event->atom, true);
    }
}
...
static void handle_client_message(XClientMessageEvent *event)
{
    ...
    if (event->message_type == ATOM(_NET_ACTIVE_WINDOW)) {
        if (window->state.is_auto_focus_allowed) {
            show_window(window);
            set_focus_window_with_frame(window);
        } else {
            reference_window(window);
            Window_last_activated = window;
        }
    ...
    /* TODO: make the removing of states configurable */
        ...
        if (atom1 == ATOM(_NET_WM_STATE_ABOVE)) {
            ...
            case _NET_WM_STATE_REMOVE: {
                ...
                /* if the window was never anything else, default to floating */
                if (window->state.previous_mode == WINDOW_MODE_FULLSCREEN) {
                    mode = WINDOW_MODE_FLOATING;
                } else {
                    mode = window->state.previous_mode;
                }
    ...
    /* request to change _NET_WM_FULLSCREEN_MONITORS */
    } else if (event->message_type == ATOM(_NET_WM_FULLSCREEN_MONITORS)) {
        XChangeProperty(display, window->reference.id,
                ATOM(_NET_WM_FULLSCREEN_MONITORS),
                XA_CARDINAL, 32, PropModeReplace,
                (unsigned char*) event->data.l, 4);
        window->properties.fullscreen_monitors.top = event->data.l[0];
        window->properties.fullscreen_monitors.bottom = event->data.l[1];
        window->properties.fullscreen_monitors.left = event->data.l[2];
        window->properties.fullscreen_monitors.right = event->data.l[3];
    }
...
void handle_event(XEvent *event)
{
    ...
    if (event->type == shape_event_base) {
        /* got a shape notify event, we could handle this and stop reshaping
         * windows that shape themselves but it is not required as of now
         */
        return;
    }
