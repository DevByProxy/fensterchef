...
/* Check if the window is covering given monitor. */
bool is_window_covering_monitor(Monitor *monitor, FcWindow *window)
{
    unsigned long monitor_area, area;
    Size overlap;
     ...
     */
    return area * 100 / monitor_area >= configuration.overlap;
}
...
void merge_monitors(Monitor *monitors)
{
    ...
    if (monitors == NULL) {
        /* always have at least one monitor */
