...
static struct test {
    ...
    { "24 run", false },
    { "a", false },
    { "relate title hey nop", true },
    { "relate title hey, class you nop", true },
    { "relate name hey, you, see nop", true },
    { "relate class hey, title hey nop", true },
...
int main(void)
{
    ...
    for (unsigned i = 0; i < SIZE(test_cases); i++) {
        ...
        if ((test_parser(parser) != OK) == test_cases[i].is_parseable) {
            ...
            if (!test_cases[i].is_parseable) {
                fprintf(stderr, "'%s' should not be parseable\n",
                        test_cases[i].input);
            } else {
